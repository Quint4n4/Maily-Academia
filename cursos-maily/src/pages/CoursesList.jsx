...
import { Card, Badge, Input, Pagination, CourseThumbnail } from '../components/ui';
  ...
  return (
    <div className="font-plus-jakarta-sans max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12 flex flex-col lg:flex-row gap-8 lg:gap-12">
      ...
      <aside className="hidden lg:flex flex-col w-64 shrink-0 gap-10">
        <div>
          <h3 className={`font-bold text-lg mb-6 flex items-center gap-2 ${ isC ? 'text-[#e6c364]' : 'text-on-surface dark:text-white' }`}>
            <span className={`material-symbols-outlined ${ isC ? 'text-[#c9a84c]' : 'text-stitch-primary dark:text-primary-fixed-dim' }`}>filter_list</span>
            Categorías
          </h3>
          <ul className="space-y-3">
            ...
            >
              <span className={`font-medium ${
                !selectedCategory 
                  ? isC ? 'font-bold text-[#e6c364]' : 'font-bold text-stitch-primary dark:text-primary-fixed-dim' 
                  : isC ? 'text-[#8a8578] group-hover:text-[#e6c364]' : 'text-on-surface-variant dark:text-gray-400 group-hover:text-stitch-primary dark:text-primary-fixed-dim'
              }`}>Todas</span>
            </li>
                ...
                {cat.courses_count != null && (
                  <span className={`text-[10px] font-bold px-2 py-1 rounded-md ${
                    selectedCategory === cat.slug 
                      ...
                      : isC ? 'bg-[#141311] text-[#8a8578] border border-[rgba(230,195,100,0.1)]' : 'bg-surface-container-highest dark:bg-gray-700 text-on-surface-variant/70 dark:text-gray-400'
          ...
          {parentCategories.length > INITIAL_CATEGORIES_SHOWN && !categoriesShowAll && (
            <button
               ...
               className={`text-xs font-bold hover:underline mt-4 ml-3 ${ isC ? 'text-[#c9a84c]' : 'text-stitch-primary dark:text-primary-fixed-dim' }`}
             >
               Mostrar más +
             </button>
          )}
        ...
        {/* Nivel de dificultad */}
        <div>
          <h3 className={`font-bold text-lg mb-6 ${ isC ? 'text-[#e6c364]' : 'text-on-surface dark:text-white' }`}>Nivel de dificultad</h3>
                ...
                <input 
                  ...
                  className={`rounded border-outline-variant w-5 h-5 ${
                    isC 
                      ...
                      : 'text-stitch-primary dark:text-primary-fixed-dim focus:ring-primary-container bg-surface-container-lowest dark:bg-gray-800'
                  }`} 
                />
                <span className={`text-sm font-medium transition-colors ${
                  selectedLevel === l 
                    ? isC ? 'text-[#e6c364] font-bold' : 'text-stitch-primary dark:text-primary-fixed-dim font-bold' 
                    : isC ? 'text-[#8a8578] group-hover:text-[#f5f0e8]' : 'text-on-surface-variant dark:text-gray-400 group-hover:text-on-surface dark:group-hover:text-white'
                }`}>
        ...
        <div className={`mt-auto p-6 rounded-[1.5rem] relative overflow-hidden hidden xl:block ${
          ...
          <div className="relative z-10">
            <p className={`text-[10px] font-black uppercase tracking-widest mb-2 ${ isC ? 'text-[#c9a84c]' : 'text-on-primary-fixed-variant dark:text-primary-fixed' }`}>Pro Access</p>
            <h4 className={`text-xl font-bold mb-4 ${ isC ? 'text-[#e6c364]' : 'text-on-primary-fixed dark:text-primary-fixed dark:text-primary-fixed' }`}>Aprendizaje Ilimitado</h4>
          ...
          <span className={`material-symbols-outlined absolute -bottom-4 -right-4 text-7xl opacity-10 rotate-12 ${ isC ? 'text-[#c9a84c]' : 'text-stitch-primary dark:text-primary-fixed-dim' }`}>school</span>
        </div>
      </aside>
              ...
              <button
                ...
                className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-bold transition-colors shadow-sm ${
                  selectedLevel === l
                    ...
                    : 'bg-surface-container-lowest dark:bg-gray-800 text-on-surface-variant dark:text-gray-400 border border-outline-variant/30'
                }`}
              >
                {l ? LEVEL_LABELS[l] : 'Todos'}
              </button>
      ...
      <section className="flex-1 min-w-0">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 sm:mb-10 gap-6">
          <div>
            <p className={`font-bold uppercase tracking-[0.2em] text-[10px] mb-2 ${ isC ? 'text-[#c9a84c]' : 'text-stitch-primary dark:text-primary-fixed-dim' }`}>Explora la excelencia</p>
            <h1 className={`text-3xl sm:text-4xl font-extrabold tracking-tight ${ isC ? 'text-[#e6c364]' : 'text-on-surface dark:text-white' }`}>Catálogo de Cursos</h1>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <div className={`flex items-center px-4 py-2.5 rounded-full border focus-within:ring-2 transition-all w-full sm:w-64 ${
              isC 
                ...
                : 'bg-surface-container-highest dark:bg-gray-700 border-outline-variant/30 ring-primary-container'
            }`}>
              ...
              <input 
                ...
                className={`bg-transparent border-none focus:ring-0 text-sm w-full outline-none ml-2 font-medium ${
                  isC 
                    ...
                    : 'text-on-surface placeholder:text-on-surface-variant/60 dark:text-gray-400'
                }`} 
              />
            </div>
            <div className="flex items-center gap-2 text-sm w-full sm:w-auto mt-2 sm:mt-0 justify-between sm:justify-start">
              <span className={`whitespace-nowrap sm:hidden lg:inline ${ isC ? 'text-[#8a8578]' : 'text-on-surface-variant/80 dark:text-gray-400' }`}>Mostrando <b>{courses.length}</b></span>
              <select aria-label="Ordenar los cursos" className={`bg-transparent border-none focus:ring-0 font-bold cursor-pointer text-sm outline-none px-2 py-1 rounded-md transition-colors ${
                isC ? 'text-[#e6c364] hover:bg-white/5' : 'text-on-surface dark:text-white hover:bg-surface-container-low'
              }`}>
          ...
          {courses.length > 0 && (
            <div className={`md:col-span-2 group relative rounded-2xl sm:rounded-[2rem] overflow-hidden transition-all duration-500 border ${
              isC 
                ...
                : 'bg-surface-container-lowest dark:bg-gray-800 shadow-[0_40px_40px_-10px_rgba(27,28,25,0.04)] hover:shadow-[0_40px_60px_-10px_rgba(27,28,25,0.08)] border-outline-variant/10'
            }`}>
              <div className="flex flex-col lg:flex-row">
                ...
                <div className="lg:w-[50%] p-8 sm:p-10 flex flex-col justify-between">
                  <div>
                    <h2 className={`text-2xl sm:text-3xl font-extrabold mb-4 leading-tight transition-colors ${ isC ? 'text-[#f5f0e8] group-hover:text-[#e6c364]' : 'text-on-surface dark:text-white group-hover:text-stitch-primary dark:text-primary-fixed-dim' }`}>{courses[0].title}</h2>
                    <p className={`leading-relaxed mb-6 opacity-80 line-clamp-3 ${ isC ? 'text-[#d0c5b2]' : 'text-on-surface-variant dark:text-gray-400' }`}>{courses[0].description}</p>
                    <div className="flex flex-wrap items-center gap-4 sm:gap-6 mb-8">
                      <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-full border flex items-center justify-center font-bold text-xs ${
                          isC ? 'bg-[#141311] border-[rgba(230,195,100,0.2)] text-[#e6c364]' : 'bg-surface-container-high border-stitch-primary/20 text-stitch-primary dark:text-primary-fixed-dim'
                        }`}>
                          {courses[0].instructor_name?.[0] || 'C'}
                        </div>
                        <span className={`text-sm font-bold ${ isC ? 'text-[#f5f0e8]' : 'text-on-surface dark:text-white' }`}>{courses[0].instructor_name}</span>
                      </div>
                  ...
                  <div className={`flex items-center justify-between pt-6 border-t ${ isC ? 'border-[rgba(77,70,55,0.2)]' : 'border-surface-container' }`}>
                    ...
                    <span className={`text-2xl sm:text-3xl font-black tracking-tight ${ isC ? 'text-[#e6c364]' : 'text-stitch-primary dark:text-primary-fixed-dim' }`}>{(!courses[0].price || Number(courses[0].price) === 0) ? 'GRATIS' : `$${Number(courses[0].price).toFixed(2)}`}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <>
              ...
              {index === 2 && (
                <div key="promo-card" className={`group rounded-2xl sm:rounded-[2rem] p-8 flex flex-col justify-between relative overflow-hidden shadow-lg ${
                  ...
                  <Link to="/subscription" className={`relative z-10 px-6 py-3 rounded-full font-extrabold text-sm self-start transition-colors shadow-sm ${
                    isC ? 'bg-[#e6c364] text-[#141311] hover:bg-[#c9a84c]' : 'bg-white text-stitch-primary dark:text-primary-fixed-dim hover:bg-primary-container hover:text-on-primary-container'
              ...
              <div key={course.id} className={`group rounded-2xl sm:rounded-[1.5rem] overflow-hidden transition-all duration-500 border flex flex-col ${
                ...
                <div className="p-6 sm:p-8 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className={`text-xl font-bold mb-3 transition-colors line-clamp-2 leading-tight ${ isC ? 'text-[#f5f0e8] group-hover:text-[#e6c364]' : 'text-on-surface dark:text-white group-hover:text-stitch-primary dark:text-primary-fixed-dim' }`}>{course.title}</h3>
                    <p className={`text-sm mb-6 ${ isC ? 'text-[#8a8578]' : 'text-outline' }`}>Inst. {course.instructor_name}</p>
                    <div className={`flex items-center gap-4 mb-8 text-xs font-bold ${ isC ? 'text-[#d0c5b2]' : 'text-on-surface-variant dark:text-gray-400' }`}>
                  ...
                  <div className={`flex items-center justify-between mt-auto pt-2 border-t ${ isC ? 'border-[rgba(77,70,55,0.2)]' : 'border-transparent' }`}>
                    <Link to={`/course/${course.id}`} className={`font-bold text-sm flex items-center gap-1 group/btn hover:underline ${ isC ? 'text-[#c9a84c]' : 'text-stitch-primary dark:text-primary-fixed-dim' }`}>
                      Ver Ficha
                      <span className="material-symbols-outlined text-sm group-hover/btn:translate-x-1 transition-transform">east</span>
                    </Link>
                    <span className={`text-2xl font-black tracking-tight ${ isC ? 'text-[#e6c364]' : 'text-stitch-primary dark:text-primary-fixed-dim' }`}>{(!course.price || Number(course.price) === 0) ? 'GRATIS' : `$${Number(course.price).toFixed(2)}`}</span>
                  </div>
                </div>
              </div>
            </>
        ...
        {!loadError && courses.length === 0 && (
          <p className={`mt-8 text-center text-sm ${isC ? 'text-[#8a8578]' : 'text-on-surface-variant dark:text-gray-400'}`}>
