...
const PieDePagina = () => (
  <footer className={`flex flex-col items-center justify-between gap-6 border-t border-academy-linea bg-white py-8 lg:flex-row ${canal} dark:border-white/10 dark:bg-academy-tinta`}>
    <div className="flex items-center gap-3">
      <img src={MARCA.logo} alt="" className="h-14 w-14" />
      <p className="font-ui text-a-13 text-academy-tinta-3 dark:text-white/50">
    ...
    <nav className="flex flex-wrap items-center justify-center gap-x-8 gap-y-2" aria-label="Pie de página">
      {PIE.map((enlace) => {
        const clases = 'font-ui text-a-13 text-academy-tinta-3 hover:text-academy-tinta dark:text-white/50 dark:hover:text-white';
        // Un enlace sin destino se pinta como texto, no como enlace muerto: el
        // aviso de privacidad todavia no existe como pagina.
        if (!enlace.a) {
          return <span key={enlace.texto} className="font-ui text-a-13 text-academy-tinta-3/50 dark:text-white/30">{enlace.texto}</span>;
        }
