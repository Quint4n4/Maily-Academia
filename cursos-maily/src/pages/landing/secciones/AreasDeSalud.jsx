...
const AreasDeSalud = ({ academias = [] }) => {
  ...
  return (
    <section id="areas" className={`bg-white py-16 lg:py-[112px] ${canal} dark:bg-academy-tinta`}>
      ...
      <div className="mx-auto grid max-w-[1200px] gap-10 sm:grid-cols-2 lg:grid-cols-3 lg:gap-0">
        {lista.map((area, indice) => (
          <article
            key={area.nombre}
            className={`lg:px-10 ${indice > 0 ? 'lg:border-l lg:border-academy-linea dark:lg:border-white/10' : ''} ${indice === 0 ? 'lg:pl-0' : ''}`}
          >
            <p className="font-display text-a-44 leading-none text-academy-oro-texto dark:text-academy-oro">
              {String(indice + 1).padStart(2, '0')}
            </p>
            <h3 className="mt-4 font-display text-a-28 text-academy-tinta dark:text-academy-crema">
              {area.nombre}
            </h3>
            <p className="mt-3 font-ui text-a-15 leading-[1.7] text-academy-tinta-3 dark:text-white/60">
              {area.texto}
            </p>
          </article>
        ))}
